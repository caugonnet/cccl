...
__device__ static unsigned long long global_time()
{
...
record_interval(unsigned long long* begin, unsigned long long* end, unsigned long long duration_ns = 100000000)
{
  *begin                   = global_time();
  unsigned long long limit = *begin + duration_ns;
  while (global_time() < limit)
  {
  }
  *end = global_time();
}
...
static void test_top_level_graph_scopes()
{
  unsigned long long* timestamps = nullptr;
  cuda_safe_call(cudaMallocManaged(&timestamps, 4 * sizeof(*timestamps)));
  for (int i = 0; i < 4; ++i)
  {
    timestamps[i] = 0;
  }
  ...
  {
    auto sibling = ctx.graph_scope();
    ctx.task(da.rw())->*[=](cudaStream_t stream, auto) {
      record_interval<<<1, 1, 0, stream>>>(timestamps, timestamps + 1);
    };
  }
  {
    auto sibling = ctx.graph_scope();
    ctx.task(db.rw())->*[=](cudaStream_t stream, auto) {
      record_interval<<<1, 1, 0, stream>>>(timestamps + 2, timestamps + 3);
    };
  }
  ctx.finalize();

  EXPECT(timestamps[0] < timestamps[3]);
  EXPECT(timestamps[2] < timestamps[1]);
  cuda_safe_call(cudaFree(timestamps));
}

static void test_scope_without_data()
{
  unsigned long long* timestamps = nullptr;
  cuda_safe_call(cudaMallocManaged(&timestamps, 2 * sizeof(*timestamps)));
  timestamps[0] = 0;
  timestamps[1] = 0;
  ...
  stackable_ctx ctx;
  {
    ...
    ctx.task()->*[=](cudaStream_t stream) {
      record_interval<<<1, 1, 0, stream>>>(timestamps, timestamps + 1, 1000000);
    };
  }
  ctx.finalize();

  EXPECT(timestamps[0] != 0);
  EXPECT(timestamps[1] != 0);
  cuda_safe_call(cudaFree(timestamps));
}
...
int main()
{
  ...
  test_top_level_graph_scopes();
  test_scope_without_data();
